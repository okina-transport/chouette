...
public class GtfsExportTripProducerTests {
	...
	private final GtfsTripProducer producer = new GtfsTripProducer(mock);
	...
	private VehicleJourney buildNeptuneObject(boolean full) {
		...
		if (full)
			neptuneObject.getAccessibilityAssessment().getAccessibilityLimitation().setWheelchairAccess(LimitationStatusEnumeration.TRUE);
		...
		for (int i = 0; i < 4; i++) {
			...
			sa.setOriginalStopId("GTFS:StopPoint:SA" + i);
