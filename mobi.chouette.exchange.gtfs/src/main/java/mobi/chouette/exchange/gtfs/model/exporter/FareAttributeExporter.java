...
public class FareAttributeExporter extends ExporterImpl<GtfsFareAttribute> implements GtfsConverter {
	...
	public static Converter<String, GtfsFareAttribute> CONVERTER = new Converter<String, GtfsFareAttribute>() {
		...
		public GtfsFareAttribute from(Context context, String input) {
			...
			bean.setPrice(FLOAT_CONVERTER.from(context, FIELDS.price, values.get(i++), true));
			...
			bean.setTransfers(ATTRIBUTETRANSFERSTYPE_CONVERTER.from(context, FIELDS.transfers, values.get(i++), false));
			bean.setAgencyId(STRING_CONVERTER.from(context, FIELDS.agency_id, values.get(i++), false));
			bean.setTransferDuration(FLOAT_CONVERTER.from(context, FIELDS.transfer_duration, values.get(i++), false));
		...
		public String to(Context context, GtfsFareAttribute input) {
			...
			values.add(FLOAT_CONVERTER.to(context, FIELDS.price, input.getPrice(), true));
	...
	static {
		...
		ExporterFactory.factories.put(FareAttributeExporter.class.getName(), factory);
	}
