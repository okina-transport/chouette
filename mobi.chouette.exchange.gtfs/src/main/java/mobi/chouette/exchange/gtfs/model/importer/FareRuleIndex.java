...
public class FareRuleIndex extends IndexImpl<GtfsFareRule> implements
        GtfsConverter {
    ...
    protected void checkRequiredFields(Map<String, Integer> fields) {
        Set<String> validFields = fields.keySet().stream()
                .filter(fieldName -> Arrays.stream(FareAttributeIndex.FIELDS.values())
            ...
            if (dao.getRouteById().containsKey(bean.getRouteId())) {
                ...
                bean.getErrors().add(
                        new GtfsException(_path, bean.getId(), getIndex(FIELDS.route_id.name()),
                                FIELDS.route_id.name(), GtfsException.ERROR.UNREFERENCED_ID, bean.getRouteId(), bean.getRouteId()));
                result = false;
            }
