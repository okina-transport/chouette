...
public interface GtfsConverter {
	...
	DefaultFieldConverter<GtfsTransfer.TransfersTypeEnum> TRANSFERTYPE_CONVERTER = new DefaultFieldConverter<GtfsTransfer.TransfersTypeEnum>() {
		@Override
		protected GtfsTransfer.TransfersTypeEnum convertFrom(String input) throws Exception {
			int ordinal = Integer.parseInt(input, 10);
			return GtfsTransfer.TransfersTypeEnum.values()[ordinal];
		}

		@Override
		protected String convertTo(GtfsTransfer.TransfersTypeEnum input) throws Exception {
			return String.valueOf(input.ordinal());
		}
	};
	...
	abstract class DefaultFieldConverter<T> extends FieldConverter<String, T> {
		@SuppressWarnings("rawtypes")
		@Override
		public synchronized T from(Context context, Enum field, String input, T value, boolean required) {
			T result = value;
			if (input != null && !input.isEmpty()) {
				try {
