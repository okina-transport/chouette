...
public class FareAttributeIndex extends IndexImpl<GtfsFareAttribute> implements
        GtfsConverter {
    ...
    protected void checkRequiredFields(Map<String, Integer> fields) {
        Set<String> validFields = fields.keySet().stream()
                .filter(fieldName -> Arrays.stream(FIELDS.values())
        ...
        if (fields.get(FIELDS.fare_id.name()) == null ||
                fields.get(FIELDS.price.name()) == null ||
                fields.get(FIELDS.currency_type.name()) == null ||
                fields.get(FIELDS.payment_method.name()) == null ||
                fields.get(FIELDS.transfers.name()) == null
        ) {// fields.get(FIELDS.agency_id.name()) == null

            String name = "";
            if (fields.get(FIELDS.fare_id.name()) == null)
                name = FIELDS.fare_id.name();
            else if (fields.get(FIELDS.price.name()) == null)
                name = FIELDS.price.name();
            else if (fields.get(FIELDS.currency_type.name()) == null)
                name = FIELDS.currency_type.name();
            else if (fields.get(FIELDS.payment_method.name()) == null)
                name = FIELDS.payment_method.name();
            else if (fields.get(FIELDS.transfers.name()) == null)
    ...
    public boolean validate(GtfsFareAttribute bean, GtfsImporter dao) {
        ...
        if (isPresent(bean.getFareId()))
            if (dao.getFareAttributeById().containsKey(bean.getFareId())) {
                ...
                bean.getErrors().add(
                        new GtfsException(_path, bean.getId(), getIndex(FIELDS.fare_id.name()),
                                FIELDS.fare_id.name(), GtfsException.ERROR.UNREFERENCED_ID, bean.getFareId(), bean.getFareId()));
                result = false;
            }
