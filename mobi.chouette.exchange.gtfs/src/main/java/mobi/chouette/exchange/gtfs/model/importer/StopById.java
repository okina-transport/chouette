...
public class StopById extends IndexImpl<GtfsStop> implements GtfsConverter {
    ...
    protected GtfsStop build(GtfsIterator reader, Context context) {
        ...
        if (value == null || value.trim().isEmpty()) {
            if (withValidation) {
                int indexLocationType = FIELDS.location_type.ordinal();
                if (array[indexLocationType] == null || array[indexLocationType].trim().isEmpty() || Integer.parseInt(array[indexLocationType]) != 1) {
    ...
    public boolean validate(GtfsStop bean, GtfsImporter dao) {
        ...
        if (!dao.getFactoryParameters().getRemoveParentStations() && isPresent(parentStationId)) {
        ...
        if (copy_bean.getStopUrl() != null) {
			for (GtfsAgency agency : dao.getAgencyById()) {
    ...
    public static class DefaultImporterFactory extends IndexFactory {
        @Override
        protected Index<GtfsStop> create(String name) throws IOException {
            return new StopById(name);
        }

        @Override
        protected Index<GtfsStop> create(String name, FactoryParameters factoryParameters) throws IOException {
