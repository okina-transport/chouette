...
public class Main {
    ...
    private void execute() {
        ...
        // fare_attributes.txt
        if (dao.hasFareAttributeImporter()) {
            parse(dao, GtfsImporter.INDEX.FARE_ID_BY_ATTRIBUTE.name(),
                    FareAttributeById.FILENAME, FareAttributeById.class);
        }

        // fare_rules.txt
        if (dao.hasFareRuleImporter()) {
            parse(dao, GtfsImporter.INDEX.FARE_ID_BY_RULE.name(),
                    FareRuleById.FILENAME, FareRuleById.class);
        }
