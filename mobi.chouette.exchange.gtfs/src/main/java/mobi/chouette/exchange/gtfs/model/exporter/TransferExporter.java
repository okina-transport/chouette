...
public class TransferExporter extends ExporterImpl<GtfsTransfer> implements GtfsConverter {
	...
	public static Converter<String, GtfsTransfer> CONVERTER = new Converter<String, GtfsTransfer>() {
		...
		public GtfsTransfer from(Context context, String input) {
			...
			int i = 0;
			bean.setFromStopId(STRING_CONVERTER.from(context, FIELDS.from_stop_id, values.get(i++), true));
			bean.setToStopId(STRING_CONVERTER.from(context, FIELDS.to_stop_id, values.get(i++), true));
			bean.setFromRouteId(STRING_CONVERTER.from(context, FIELDS.from_route_id, values.get(i++), false));
			bean.setToRouteId(STRING_CONVERTER.from(context, FIELDS.to_route_id, values.get(i++), false));
			bean.setFromTripId(STRING_CONVERTER.from(context, FIELDS.from_trip_id, values.get(i++), false));
			bean.setToTripId(STRING_CONVERTER.from(context, FIELDS.to_trip_id, values.get(i++), false));
			bean.setTransferType(TRANSFERTYPE_CONVERTER.from(context, FIELDS.transfer_type, values.get(i++), true));
			bean.setMinTransferTime(INTEGER_CONVERTER.from(context, FIELDS.min_transfer_time, values.get(i++), false));
		...
		public String to(Context context, GtfsTransfer input) {
			...
			values.add(STRING_CONVERTER.to(context, FIELDS.from_stop_id, input.getFromStopId(), true));
			values.add(STRING_CONVERTER.to(context, FIELDS.to_stop_id, input.getToStopId(), true));
			values.add(STRING_CONVERTER.to(context, FIELDS.from_route_id, input.getFromRouteId(), false));
