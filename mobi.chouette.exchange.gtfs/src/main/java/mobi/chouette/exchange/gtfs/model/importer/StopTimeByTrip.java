...
//	private String _stopId = null;
    ...
    protected GtfsStopTime build(GtfsIterator reader, Context context) {
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.trip_id.name(), value, _bean);
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.stop_id.name(), value, _bean);
        if (value == null || value.trim().isEmpty()) {
            if (withValidation)
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.stop_sequence.name(), value, _bean);
        if (value == null || value.trim().isEmpty()) {
            if (withValidation)
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.arrival_time.name(), value, _bean);
        if (value == null || value.trim().isEmpty()) {
            noArrivalTime = true;
        } else {
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.departure_time.name(), value, _bean);
        if (value == null || value.trim().isEmpty()) {
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.stop_headsign.name(), value, _bean);
        if (value != null && !value.trim().isEmpty()) {
            _bean.setStopHeadsign(STRING_CONVERTER.from(context, FIELDS.stop_headsign, value, false));
        }

        value = _array[i++];
        testExtraSpace(FIELDS.pickup_type.name(), value, _bean);
        if (value != null && !value.trim().isEmpty()) {
            try {
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.drop_off_type.name(), value, _bean);
        if (value != null && !value.trim().isEmpty()) {
            try {
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.shape_dist_traveled.name(), value, _bean);
        if (value != null && !value.trim().isEmpty()) {
            try {
        ...
        value = _array[i++];
        testExtraSpace(FIELDS.timepoint.name(), value, _bean);
        if (value != null && !value.trim().isEmpty()) {
            try {
    ...
    public boolean validate(GtfsStopTime bean, GtfsImporter dao) {
        ...
        if (!dao.hasTripImporter() ) {
            return endValidate(bean, result);
        }


        try {

            if (dao.getTripById().getLength() != 0 && !dao.getTripById().containsKey(tripId)) {
            ...
            log.info("Trip id from stop_times can't be checked because of an exception in tripImporter", ex);
    ...
    private boolean endValidate(GtfsStopTime bean, boolean result) {
        if (result) {
