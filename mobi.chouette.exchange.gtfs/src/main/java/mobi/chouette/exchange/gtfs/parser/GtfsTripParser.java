...
import mobi.chouette.exchange.gtfs.model.GtfsTransfer.TransfersTypeEnum;
...
public class GtfsTripParser implements Parser, Validator, Constant {
    ...
    private void validateStopTimes(Context context) throws Exception {
        ...
        if (importer.hasStopTimeImporter()) { // the file "stop_times.txt"
            ...
            if (stopTimeParser == null) { // importer.getStopTimeByTrip() fails
                ...
                gtfsValidationReporter.throwUnknownError(context, new Exception("Cannot instantiate StopTimeByTrip class"), GTFS_STOP_TIMES_FILE);
            } else {
            ...
            if (stopTimeParser.getLength() == 0) {
                gtfsValidationReporter.reportError(context, new GtfsException(GTFS_STOP_TIMES_FILE, 1, null, GtfsException.ERROR.FILE_WITH_NO_ENTRY, null, null), GTFS_STOP_TIMES_FILE);
            } else {
            ...
            for (String tripId : tripIds) {
                ...
                for (GtfsStopTime bean : stopTimes) {
                    ...
                    if (params.isRoutesReorganization()) {
                        GtfsStopTime tmp = new GtfsStopTime(bean.getTripId(), null, null, bean.getStopId(), stopSequence, null, null, null, null, null);
                    ...
                    if (stopSequence != null) {
                        if (stopSequences.containsKey(stopSequence)) {
                            ...
                            gtfsValidationReporter.validate(context, GTFS_STOP_TIMES_FILE, GtfsException.ERROR.DUPLICATE_STOP_SEQUENCE);
                        }
                    }
                }
                ...
                if (params.isRoutesReorganization()) {
                    ...
                    for (GtfsStopTime stopTime : tripIdStopTimes) {
                        if (prevStopId.equals(stopTime.getStopId())) {
                            ...
                            gtfsValidationReporter.reportError(context, new GtfsException(stopTimeParser.getPath(), stopTime.getId(), stopTimeParser.getIndex(StopTimeByTrip.FIELDS.stop_id.name()), StopTimeByTrip.FIELDS.trip_id.name() + ", " + StopTimeByTrip.FIELDS.stop_sequence.name(), GtfsException.ERROR.DUPLICATE_CONSECUTIVE_STOP_TIME, null, stopTimePrimaryKey), GTFS_STOP_TIMES_FILE);
            ...
            for (GtfsStop bean : importer.getStopById()) {
                if (LocationType.Stop.equals(bean.getLocationType())) {
                    if (stopIds.add(bean.getStopId())) {
                        unusedId = false;
                        gtfsValidationReporter.reportError(context, new GtfsException(GTFS_STOPS_FILE, i, StopById.FIELDS.stop_id.name(), GtfsException.ERROR.UNUSED_ID, null, bean.getStopId()), GTFS_STOPS_FILE);
                    }
                }
                i++;
            }
            ...
            gtfsValidationReporter.reportError(context, new GtfsException(GTFS_STOP_TIMES_FILE, 1, null, GtfsException.ERROR.MISSING_FILE, null, null), GTFS_STOP_TIMES_FILE);
        }
    }
    ...
    private void findTripIdsWithSameTimes(Index<GtfsStopTime> stopTimeParser, Context context) {
        ...
        tripTimeMap.forEach((tripId, times) -> timeTripIdMap.merge(times, tripId, (existingIds, newId) -> existingIds + "+" + newId));
    ...
    private void validateShapes(Context context) throws Exception {
        ...
        if (importer.hasShapeImporter()) {
            ...
            if (shapeParser == null) { // importer.getShapeById() fails for any
                // other reason
                gtfsValidationReporter.throwUnknownError(context, new Exception("Cannot instantiate ShapeById class"), GTFS_SHAPES_FILE);
            } else {
            ...
            if (shapeParser.getLength() == 0) {
                gtfsValidationReporter.reportError(context, new GtfsException(GTFS_SHAPES_FILE, 1, null, GtfsException.ERROR.OPTIONAL_FILE_WITH_NO_ENTRY, null, null), GTFS_SHAPES_FILE);
            } else {
                    ...
                    for (GtfsShape bean : shapes) {
                        ...
                        if (stopSequence != null) {
                            if (shapeSequences.containsKey(stopSequence)) {
                                ...
                                gtfsValidationReporter.validate(context, GTFS_SHAPES_FILE, GtfsException.ERROR.DUPLICATE_STOP_SEQUENCE);
                            }
                        }
                    }
                }
            ...
            gtfsValidationReporter.reportError(context, new GtfsException(GTFS_SHAPES_FILE, 1, null, GtfsException.ERROR.MISSING_OPTIONAL_FILE, null, null), GTFS_SHAPES_FILE);
        }
    }
    ...
    private void validateTrips(Context context) throws Exception {
        ...
        if (importer.hasTripImporter()) { // the file "trips.txt" exists ?
            ...
            if (tripParser == null) { // importer.getTripById() fails for any
                // other reason
                gtfsValidationReporter.throwUnknownError(context, new Exception("Cannot instantiate TripById class"), GTFS_TRIPS_FILE);
            } else {
            ...
            if (tripParser.getLength() == 0) {
                gtfsValidationReporter.reportError(context, new GtfsException(GTFS_TRIPS_FILE, 1, null, GtfsException.ERROR.FILE_WITH_NO_ENTRY, null, null), GTFS_TRIPS_FILE);
            } else {
            ...
            for (GtfsRoute bean : importer.getRouteById()) {
                if (routeIds.add(bean.getRouteId())) {
                    unsuedId = false;
                    gtfsValidationReporter.reportError(context, new GtfsException(GTFS_ROUTES_FILE, i, RouteById.FIELDS.route_id.name(), GtfsException.ERROR.UNUSED_ID, null, bean.getRouteId()), GTFS_TRIPS_FILE);
                }
                i++;
            }
            if (unsuedId)
            ...
            gtfsValidationReporter.reportError(context, new GtfsException(GTFS_TRIPS_FILE, 1, null, GtfsException.ERROR.MISSING_FILE, null, null), GTFS_TRIPS_FILE);
        }
    }
    ...
    private void filterTripsWithSameServiceIdAndPutToContext(Context context, Index<GtfsTrip> tripParser) {
        ...
        for (String tripIds : tripIdsWithSameTimesList) {
            ...
            if (splitTripIds.length >= 3) {
                ...
                List<String> largestGroup = serviceIdToTripIds.values().stream().max(Comparator.comparingInt(List::size)).orElse(new ArrayList<>());
    ...
    private void validateFrequencies(Context context) throws Exception {
        ...
        if (importer.hasFrequencyImporter()) {
            ...
            if (frequencyParser == null) { // importer.getFrequencyByTrip()
                ...
                gtfsValidationReporter.throwUnknownError(context, new Exception("Cannot instantiate FrequencyByTrip class"), GTFS_FREQUENCIES_FILE);
            } else {
            ...
            if (frequencyParser.getLength() == 0) {
                ...
                gtfsValidationReporter.reportError(context, new GtfsException(GTFS_FREQUENCIES_FILE, 1, null, GtfsException.ERROR.OPTIONAL_FILE_WITH_NO_ENTRY, null, null), GTFS_FREQUENCIES_FILE);
            } else {
            ...
            gtfsValidationReporter.reportError(context, new GtfsException(GTFS_FREQUENCIES_FILE, 1, null, GtfsException.ERROR.MISSING_OPTIONAL_FILE, null, null), GTFS_FREQUENCIES_FILE);
        }
    }
    ...
    public void parse(Context context) throws Exception {
        ...
        for (GtfsTrip gtfsTrip : gtfsTrips.values(gtfsRouteId)) {
            ...
            String objectId = ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), VehicleJourney.VEHICLEJOURNEY_KEY, gtfsTrip.getTripId());
            ...
            if (StringUtils.isNotBlank(configuration.getExternalRefField())) {
                KeyValue externalRef = new KeyValue();
                externalRef.setKey(EXTERNAL_REF);
                externalRef.setValue(getFieldValue(gtfsTrip, configuration.getExternalRefField()));
                externalRef.setTypeOfKey("ALTERNATIVE_IDENTIFIER");
                vehicleJourney.getKeyValues().add(externalRef);
            }

            if (StringUtils.isNotBlank(configuration.getDriverControllerCodeField())) {
                KeyValue dccf = new KeyValue();
                dccf.setKey(DRIVER_CONTROLLER_CODE);
                dccf.setValue(getFieldValue(gtfsTrip, configuration.getDriverControllerCodeField()));
            ...
            for (GtfsStopTime gtfsStopTime : importer.getStopTimeByTrip().values(gtfsTrip.getTripId())) {
                ...
                VehicleJourneyAtStopWrapper vehicleJourneyAtStop = new VehicleJourneyAtStopWrapper(stopId, gtfsStopTime.getStopSequence(), gtfsStopTime.getShapeDistTraveled(), gtfsStopTime.getDropOffType(), gtfsStopTime.getPickupType(), gtfsStopTime.getStopHeadsign());
            ...
            // Timetable
            String timetableId = ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), Timetable.TIMETABLE_KEY, gtfsTrip.getServiceId());
            ...
            if (journeyPattern == null) {
                journeyPattern = createJourneyPattern(referential, configuration, gtfsTrip, importer, vehicleJourney, objectIdKey.toString(), journeyPatternByStopSequence, position);
            }
    ...
    private void createInterchanges(Referential referential, GtfsImporter importer, GtfsImportParameters configuration, GtfsTrip gtfsTrip, VehicleJourney vehicleJourney) {
        ...
        for (GtfsTransfer gtfsTransfer : importer.getTransferByFromTrip().values(gtfsTrip.getTripId())) {
            if (gtfsTransfer.getFromTripId() != null && gtfsTransfer.getToTripId() != null) {
                ...
                String feederStopAreaId = ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), "Quay", gtfsTransfer.getFromStopId());
        ...
        for (GtfsTransfer gtfsTransfer : importer.getTransferByToTrip().values(gtfsTrip.getTripId())) {
            if (gtfsTransfer.getFromTripId() != null && gtfsTransfer.getToTripId() != null) {
                ...
                String consumerStopAreaId = ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), "Quay", gtfsTransfer.getToStopId());
    ...
    protected Interchange createInterchange(Referential referential, GtfsImportParameters configuration, GtfsTransfer gtfsTransfer) {
        String partialId = StringUtils.join(new String[]{gtfsTransfer.getFromStopId(), gtfsTransfer.getToStopId(), gtfsTransfer.getFromRouteId(), gtfsTransfer.getToRouteId(), gtfsTransfer.getFromTripId(), gtfsTransfer.getToTripId(),}, "_");
        String objectId = ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), Interchange.INTERCHANGE_KEY, partialId);
    ...
    private void createJourneyFrequencies(Referential referential, GtfsImporter importer, GtfsImportParameters configuration, GtfsTrip gtfsTrip, VehicleJourney vehicleJourney) {
        int count = 0;
        for (GtfsFrequency frequency : importer.getFrequencyByTrip().values(gtfsTrip.getTripId())) {
            ...
            String timeBandObjectId = ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), Timeband.TIMETABLE_KEY, gtfsTrip.getTripId() + "-" + count++);
            ...
            String journeyFrequencyObjectId = ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), "HeadwayJourney", gtfsTrip.getTripId() + "-" + count++);
    ...
    private JourneyPattern createJourneyPattern(Referential referential, GtfsImportParameters configuration, GtfsTrip gtfsTrip, GtfsImporter importer, VehicleJourney vehicleJourney, String objectIdKey, Map<String, JourneyPattern> journeyPatternByStopSequence, Integer position) {
        ...
        // Shape -> routeSections
        if (configuration.isImportShapesFile() && gtfsTrip.getShapeId() != null && !gtfsTrip.getShapeId().isEmpty() && importer.getShapeById().containsKey(gtfsTrip.getShapeId())) {
    ...
    private void addSyntheticDestinationDisplayIfMissingOnFirstStopPoint(GtfsImportParameters configuration, Referential referential, JourneyPattern jp) {
        ...
        if (departureStopPoint.getDestinationDisplay() == null) {
            ...
            DestinationDisplay destinationDisplay = ObjectFactory.getDestinationDisplay(referential, ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), DestinationDisplay.DESTINATIONDISPLAY_KEY, journeyPatternId + "-" + stopPointId));

            if (jp.getArrivalStopPoint() != null && jp.getArrivalStopPoint().getScheduledStopPoint() != null && jp.getArrivalStopPoint().getScheduledStopPoint().getContainedInStopAreaRef() != null && jp.getArrivalStopPoint().getScheduledStopPoint().getContainedInStopAreaRef().getObject() != null) {
     ...
     * @param configuration
     */
    private void createStopPoint(Route route, JourneyPattern journeyPattern, List<VehicleJourneyAtStop> list, Referential referential, GtfsImportParameters configuration, Integer position) {
        ...
        for (VehicleJourneyAtStop vehicleJourneyAtStop : list) {
            ...
            String baseKey = route.getObjectId().replace(Route.ROUTE_KEY, StopPoint.STOPPOINT_KEY) + "a" + stopIdKeyFragment.trim();
            ...
            String stopAreaId = ObjectIdUtil.toStopAreaId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), "Quay", wrapper.stopId);
            ...
            if (wrapper.stopHeadsign != null) {
                DestinationDisplay destinationDisplay = ObjectFactory.getDestinationDisplay(referential, ObjectIdUtil.composeObjectId(configuration.isSplitIdOnDot(), configuration.getObjectIdPrefix(), DestinationDisplay.DESTINATIONDISPLAY_KEY, stopKey));
    ...
    public String getFieldValue(GtfsTrip gtfsTrip, String field) {
        switch (TripIndex.FIELDS.valueOf(field)) {
            case route_id:
                return gtfsTrip.getRouteId();
            case service_id:
                return gtfsTrip.getServiceId();
            case trip_id:
                return gtfsTrip.getTripId();
            case trip_headsign:
                return gtfsTrip.getTripHeadSign();
            case trip_short_name:
                return gtfsTrip.getTripShortName();
            case direction_id:
                return gtfsTrip.getDirectionId() == null ? "" :
                        Integer.toString(gtfsTrip.getDirectionId().ordinal());
            case block_id:
                return gtfsTrip.getBlockId();
            case shape_id:
                return gtfsTrip.getShapeId();
            case wheelchair_accessible:
                return gtfsTrip.getWheelchairAccessible() == null ? "" :
                        Integer.toString(gtfsTrip.getWheelchairAccessible().ordinal());
            case bikes_allowed:
                return gtfsTrip.getBikesAllowed() == null ? "" :
                        Integer.toString(gtfsTrip.getBikesAllowed().ordinal());
            default:
