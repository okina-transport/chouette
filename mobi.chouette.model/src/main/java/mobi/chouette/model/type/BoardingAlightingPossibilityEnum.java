...
public enum BoardingAlightingPossibilityEnum
{
   ...
   public static BoardingAlightingPossibilityEnum fromDropOffAndPickUp(DropOffTypeEnum dropOff,PickUpTypeEnum pickUp){
      ...
      for (BoardingAlightingPossibilityEnum value : BoardingAlightingPossibilityEnum.values()) {
         if (value.getDropOffType().equals(finalDropOff) && value.getPickUpType().equals(finalPickUp)){
            return value;
         }
      }

      throw new IllegalArgumentException("Boarding alighting not found for parameters:" + finalDropOff + ", " + finalPickUp);
   }
