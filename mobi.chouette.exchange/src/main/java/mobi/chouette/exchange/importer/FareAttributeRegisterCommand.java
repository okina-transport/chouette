...
public class FareAttributeRegisterCommand implements Command {
    ...
    public boolean execute(Context context) throws Exception {
        ...
        FareAttribute newValue = referential.getFareAttributes().values().iterator().next();
        context.put(CURRENT_FARE_ATTRIBUTE_ID, newValue.getObjectId());

        log.info("register fare attribute : " + newValue.getObjectId());
        try {
            optimiser.initialize(cache, referential);

            FareAttribute oldValue = cache.getFareAttributes().get(newValue.getObjectId());
            Agency oldAgencyValue = cache.getAgencies().get(newValue.getAgency().getAgencyId());

            newValue.setAgency(oldAgencyValue);

            if (oldValue.getId() == null) {
                fareAttributeDAO.create(oldValue);
            } else {
                fareAttributeUpdater.update(context, oldValue, newValue);
            }
            ...
            reporter.addObjectReport(context, newValue.getObjectId(),
                    OBJECT_TYPE.FARE_ATTRIBUTE, NamingUtil.getName(newValue), OBJECT_STATE.ERROR, IO_TYPE.INPUT);
            if (ex.getCause() != null) {
                ...
                if (e instanceof SQLException) {
                    ...
                    reporter.addErrorToObjectReport(context, newValue.getObjectId(), OBJECT_TYPE.FARE_ATTRIBUTE, ERROR_CODE.WRITE_ERROR, e.getMessage());

                } else {
                    reporter.addErrorToObjectReport(context, newValue.getObjectId(), OBJECT_TYPE.FARE_ATTRIBUTE, ERROR_CODE.INTERNAL_ERROR, e.getMessage());
                }
            } else {
                reporter.addErrorToObjectReport(context, newValue.getObjectId(), OBJECT_TYPE.FARE_ATTRIBUTE, ERROR_CODE.INTERNAL_ERROR, ex.getMessage());
            }
            throw ex;
        } finally {
