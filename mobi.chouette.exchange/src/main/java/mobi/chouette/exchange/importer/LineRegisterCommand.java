...
public class LineRegisterCommand implements Command {
    ...
    public boolean execute(Context context) throws Exception {
        ...
        if (importParameter.isKeepObsoleteLines() || isLineValidInFuture(newValue)) {
            ...
            boolean shouldMapIds = Boolean.parseBoolean(System.getProperty(checker.getContext() + PropertyNames.STOP_PLACE_ID_MAPPING)) && importParameter.isStopAreaRemoteIdMapping();
            if (shouldMapIds) {
            ...
            try {
                ...
                if (optimized) {
                    ...
                    List<VehicleJourneyAtStopTranslation> vjasTranslations = new ArrayList<>();

                    final List<String> vjObjectIdsToDelete = referential.getVehicleJourneys().values().stream().map(VehicleJourney::getObjectId).collect(Collectors.toList());
     ...
     *
     * @param line the line for which we need to remove empty routes
     */
     ...
     *
     * @param vehicleJourney          VehicleJourney that contains old data from DB
     * @param newVehicleJourneyAtStop newVehicleJourney
     * @return
     */
    private Optional<BoardingAlightingPossibilityEnum> getActualBoardingAlightingPossibility(VehicleJourney vehicleJourney, VehicleJourneyAtStop newVehicleJourneyAtStop) {
        return vehicleJourney.getVehicleJourneyAtStops().stream().filter(currentVehicleJourneyAtStop -> currentVehicleJourneyAtStop.getStopPoint().equals(newVehicleJourneyAtStop.getStopPoint()) && currentVehicleJourneyAtStop.getVehicleJourney().equals(newVehicleJourneyAtStop.getVehicleJourney())).map(VehicleJourneyAtStop::getBoardingAlightingPossibility).filter(Objects::nonNull).findFirst();
    }
