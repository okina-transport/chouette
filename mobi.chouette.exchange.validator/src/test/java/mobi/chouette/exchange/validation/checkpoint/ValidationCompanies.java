...
public class ValidationCompanies extends AbstractTestValidation {
	private CompanyCheckPoints checkPoint = new CompanyCheckPoints();
	private ValidationParameters fullparameters;
	private Company bean1;
	private Company bean2;
	private List<Company> beansFor4 = new ArrayList<>();
	...
	@Test(groups = { "company" }, description = "4-Company-1 no test",priority=1)
	public void verifyTest4_1_notest() throws Exception {
		// 4-Company-1 : check columns
		...
		data.getCompanies().addAll(beansFor4);
	...
	@Test(groups = { "company" }, description = "4-Company-1 unicity",priority=2)
	public void verifyTest4_1_unique() throws Exception {
		// 4-Company-1 : check columns
		log.info(Color.BLUE + "4-Company-1 unicity" + Color.NORMAL);
		Context context = initValidatorContext();
		Assert.assertNotNull(fullparameters, "no parameters for test");
		...
		CheckPointReport checkPointReport = report.findCheckPointReportByName("4-Company-1");
