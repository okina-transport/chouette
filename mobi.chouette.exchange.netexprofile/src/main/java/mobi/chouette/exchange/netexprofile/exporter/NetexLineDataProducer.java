...
public class NetexLineDataProducer extends NetexProducer implements Constant {
    ...
    private static final OrganisationProducer organisationProducer = new OrganisationProducer();
    private static final StopPlaceProducer stopPlaceProducer = new StopPlaceProducer();
    private static final NetworkProducer networkProducer = new NetworkProducer();
    private static final LineProducer lineProducer = new LineProducer();
    private static final RouteProducer routeProducer = new RouteProducer();
    private static final JourneyPatternProducer journeyPatternProducer = new JourneyPatternProducer();
    private static final CalendarProducer calendarProducer = new CalendarProducer();
    private static final ServiceJourneyProducer serviceJourneyProducer = new ServiceJourneyProducer();
    private static final ServiceJourneyInterchangeProducer serviceJourneyInterchangeProducer = new ServiceJourneyInterchangeProducer();
    private static final BrandingProducer brandingProducer = new BrandingProducer();
            ...
            for (JourneyPattern journeyPattern : route.getJourneyPatterns()) {
                for (StopPoint stopPoint : journeyPattern.getStopPoints()) {
                    if (stopPoint != null) {
                        ...
                        if (dd != null) {
                            addDestinationDisplay(context, dd, exportableNetexData);
                        }
                    }
                }
            }
        ...
        if (!exportableNetexData.getSharedDestinationDisplays().containsKey(dd.getObjectId())) {
            ...
            if (dd.getVias() != null && dd.getVias().size() > 0) {
                ...
                for (mobi.chouette.model.DestinationDisplay via : dd.getVias()) {
                    ...
                    addDestinationDisplay(context, via, exportableNetexData);
