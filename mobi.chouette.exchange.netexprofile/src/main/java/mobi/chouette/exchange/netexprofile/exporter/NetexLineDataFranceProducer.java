...
public class NetexLineDataFranceProducer extends NetexProducer implements Constant {
    ...
    private PassengerStopAssignment createPassengerStopAssignment(mobi.chouette.model.ScheduledStopPoint scheduledStopPoint, String stopAssignmentId, NetexprofileExportParameters parameters) {
        ...
        if (isSet(scheduledStopPoint.getContainedInStopAreaRef().getObject())) {
			...
			boolean isAutoGenerated = containedInStopArea.getKeyValues().stream().anyMatch(keyValue -> keyValue.getKey().equals("automaticaly-created-missing-quay"));

			if (parameters.isExportGeneratedMissingQuays() && isAutoGenerated) {
			...
			} else if (!parameters.isExportGeneratedMissingQuays() && containedInStopArea.getParent() != null){
				StopPlaceRefStructure stopPlaceRefStructure = netexFactory.createStopPlaceRefStructure();
				NetexProducerUtils.populateReference(containedInStopArea.getParent(), stopPlaceRefStructure, parameters.isExportStops());
				stopPlaceRefStructure.setVersionRef("any");
				passengerStopAssignment.setStopPlaceRef(netexObjectFactory.createStopPlaceRef(stopPlaceRefStructure));
			}
        }
