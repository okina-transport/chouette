...
public class ServiceCalendarFrameParser extends NetexParser implements Parser, Constant {
    ...
    public void parse(Context context) throws Exception {
        ...
        if (serviceCalendarFrame.getOperatingPeriods() != null) {
            for (OperatingPeriod_VersionStructure operatingPeriodStruct : serviceCalendarFrame.getOperatingPeriods().getOperatingPeriodOrUicOperatingPeriod()) {
                ...
                if (operatingPeriodStruct instanceof OperatingPeriod) {
                    ...
                    UicOperatingPeriod uicOpPeriod = (UicOperatingPeriod) operatingPeriodStruct;
                    NetexObjectUtil.addOperatingPeriodRef(netexReferential, uicOpPeriod.getId(), uicOpPeriod);
                }
            }
        }
    ...
    private void convertCalendarToTimetable(Context context) throws Exception {
        ...
        for (DayTypeAssignment dayTypeAssignment : netexReferential.getDayTypeAssignments()) {
            ...
            if (dayTypeAssignment.getDate() != null) {
                ...
                Optional<OperatingPeriod> operatingPeriodOpt = NetexObjectUtil.getOperatingPeriod(netexReferential, operatingPeriodIdRef);

                OperatingDayRefStructure fromDay = null;
                LocalDateTime fromDate = null;
                OperatingDayRefStructure toDay = null;
                LocalDateTime toDate = null;

                if (operatingPeriodOpt.isPresent()) {
                    OperatingPeriod operatingPeriod = operatingPeriodOpt.get();
                    fromDay = operatingPeriod.getFromOperatingDayRef();
                    fromDate = operatingPeriod.getFromDate();
                    toDay = operatingPeriod.getToOperatingDayRef();
                    toDate = operatingPeriod.getToDate();

                } else {
                    Optional<UicOperatingPeriod> uicOpPeriocOpt = NetexObjectUtil.getUicOperatingPeriod(netexReferential, operatingPeriodIdRef);
                    if (uicOpPeriocOpt.isPresent()) {
                        UicOperatingPeriod uicOpPeriod = uicOpPeriocOpt.get();
                        fromDay = uicOpPeriod.getFromOperatingDayRef();
                        fromDate = uicOpPeriod.getFromDate();
                        toDay = uicOpPeriod.getToOperatingDayRef();
                        toDate = uicOpPeriod.getToDate();

                        if (!StringUtils.isEmpty(uicOpPeriod.getValidDayBits())) {
                            addCalendarDates(timetable, uicOpPeriod);
                            //calendar dates have been added to time table using validBits. No need to add a period. Must go to next timetable
                            continue;
                        }
                    }
                }
    ...
    private void addCalendarDates(Timetable timetable, UicOperatingPeriod uicOpPeriod) {
        if (uicOpPeriod.getFromDate() == null) {
            return;
        }
        ...
        for (int dayIndex = 0; dayIndex < uicOpPeriod.getValidDayBits().length(); dayIndex++) {
            ...
            calendarDay.setDate(currDate);
            calendarDay.setIncluded(currBit == '1');
            timetable.getCalendarDays().add(calendarDay);
    ...
    private ValidBetween getValidBetweenForFrame(Context context) {
        ...
        Context publicationDeliveryContext = getLocalContext(context, PublicationDeliveryParser.LOCAL_CONTEXT);
        ...
        ValidBetween defaultValidBetween = new ValidBetween();
        defaultValidBetween.setId(UUID.randomUUID().toString());
        defaultValidBetween.setFromDate(LocalDateTime.of(1980, 01, 01, 0, 0));
        defaultValidBetween.setToDate(LocalDateTime.of(2999, 01, 01, 0, 0));
        return defaultValidBetween;
    }
