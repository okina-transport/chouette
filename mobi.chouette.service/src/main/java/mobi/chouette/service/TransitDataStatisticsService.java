...
public class TransitDataStatisticsService {
    ...
    void categorizeValidity(LineStatistics lineStats, Date startDate, Map<Integer, String> minDaysValidityCategories) {
        ...
        LocalDate startDateLocal = TimeUtil.toLocalDate(startDate);
                        ...
                        if (t.getCalendarDays() != null) {
                            ...
                            for (CalendarDay dayDelete : daystoDelete) {
                                if (p.getStartDate().equals(dayDelete.getDate())) {
                                    newDateStart.setTime(TimeUtil.toDate(p.getStartDate()));
                                    newDateStart.add(Calendar.DATE, 1);
                                    newDateStartValued = true;
                                }

                                if (newDateStartValued && newDateStart.getTime().equals(TimeUtil.toDate(dayDelete.getDate()))) {
                                    newDateStart.add(Calendar.DATE, 1);
                                }
                            }
                            ...
                            for (CalendarDay dayDeleteReverse : daysToDeleteReverse) {
                                if (p.getEndDate().equals(dayDeleteReverse.getDate())) {
                                    newDateEnd.setTime((TimeUtil.toDate(p.getEndDate())));
                                    newDateEnd.add(Calendar.DATE, -1);
                                    newDateEndValued = true;
                                }

                                if (newDateEndValued && newDateEnd.getTime().equals(TimeUtil.toDate(dayDeleteReverse.getDate()))) {
                                    newDateEnd.add(Calendar.DATE, -1);
                                }
                            }
                            ...
                            for (CalendarDay dayDelete : daystoDelete) {
                                ...
                                if (newDateStart.getTime().compareTo(TimeUtil.toDate(dayDelete.getDate())) <= 0 && newDateEnd.getTime().compareTo(TimeUtil.toDate(dayDelete.getDate())) > 0) {
                                    periodsToDelete.add(new Period(newDateStart.getTime(), newDateEnd.getTime()));
                                    Calendar dateEnd = Calendar.getInstance();
                                    Calendar dateStart = Calendar.getInstance();

                                    if (newDateStart.getTime().compareTo(TimeUtil.toDate(dayDelete.getDate())) < 0) {
                                        dateEnd.setTime(TimeUtil.toDate(dayDelete.getDate()));
                                        dateEnd.add(Calendar.DATE, -1);

                                        timetable.getPeriods().add(new Period(newDateStart.getTime(), dateEnd.getTime()));
                                    }

                                    dateStart.setTime(TimeUtil.toDate(dayDelete.getDate()));
                                    dateStart.add(Calendar.DATE, 1);

                                    timetable.getPeriods().add(new Period(dateStart.getTime(), newDateEnd.getTime()));
                                    newDateStart = dateStart;
                                    newDateStartValued = true;
                                } else if (!newDateStart.getTime().equals(TimeUtil.toDate(p.getStartDate())) || !newDateEnd.getTime().equals(TimeUtil.toDate(p.getEndDate()))) {
                                    periodsToDelete.add(new Period(p.getStartDate(), p.getEndDate()));
    ...
    private Period calculatePeriodFromCalendarDaysPattern(Collection<CalendarDay> calendarDays) {
        ...
        Set<java.time.LocalDate> includedDays = calendarDays.stream().filter(CalendarDay::getIncluded)
                .map(c -> java.time.LocalDate.of(c.getDate().getYear(), c.getDate().getMonth(), c.getDate().getDayOfMonth())).collect(Collectors.toSet());
