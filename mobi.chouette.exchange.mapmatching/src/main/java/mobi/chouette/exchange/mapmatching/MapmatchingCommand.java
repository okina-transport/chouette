...
public class MapmatchingCommand implements Command, Constant {
	...
	public GeoJSON loadSectionRoute(JourneyPattern journeyPattern, List<LatLngMapMatching> listStopPoints, ImportGenerateMapMatching mapMatchingType) throws Exception {
		...
		ProfileOSRMJourneyPattern profile = addProfileJourneyPattern(journeyPattern, osrmProfile, osrmResponseMultiLines);
		...
		for (List<LatLngMapMatching> sectionStopPoint : listOfSectionStopPoints) {
			...
			StopPoint stopPointDeparture = stopPoints.stream().filter(sp -> sp.getId().equals(departureStopPoint.getStopPoint().getId())).findAny().orElseThrow(() -> new Exception("StopPoint not found in the list of points provided."));

			StopPoint stopPointArrival = stopPoints.stream().filter(sp -> sp.getId().equals(arrivalStopPoint.getStopPoint().getId())).findAny().orElseThrow(() -> new Exception("StopPoint not found in the list of points provided."));
			...
			if (OSRMProfile.AIR.equals(osrmProfile) || OSRMProfile.FERRY.equals(osrmProfile) || OSRMProfile.METRO.equals(osrmProfile)) {
				...
				ProfileOSRMInterStopJourneyPattern profileInterStopJP = getInterStopInformations(osrmResponseMultiLines,
						...
						stopPointDeparture, stopPointArrival, indexStart, indexNext, profile.getProfileOSRMInterStopJourneyPatternList());
	...
	protected List<LatLngMapMatching> getPoints(JourneyPattern journeyPattern) {
		if (journeyPattern.getLatLngMapMatching().isEmpty()) {
			return stopPointDAO.getStopPointsofJourneyPattern(journeyPattern).stream().map(LatLngMapMatching::new).collect(toList());
		} else {
	...
	private com.vividsolutions.jts.geom.LineString correctSectionLineWithJourneyPatternLine(com.vividsolutions.jts.geom.LineString sectionLine, com.vividsolutions.jts.geom.LineString jpLine) throws Exception {
		...
		int jpIndexMatchingSectionStart = findClosestJPCoordinateIndex(jpCoords, sectionCoords, 0, 0, 10).orElseThrow(() -> new Exception("Parsing OSRM results: Section and Journey Pattern path are incompatible"));

		int jpIndexMatchingSectionEnd = findClosestJPCoordinateIndex(jpCoords, sectionCoords, sectionCoords.length - 1, jpIndexMatchingSectionStart + 1, 10).orElseThrow(() -> new Exception("Parsing OSRM results: Section and Journey Pattern path are incompatible"));
